...
The build script we'll be using is contained in the ADON github repository ([contrib/gitian-build.py](https://github.com/AdonisExchange/ADON/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ADON repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/AdonisExchange/ADON/master/contrib/gitian-build.py
