...
This document provides guidelines for internationalization of the ADON Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to ADON network</source>
        <translation>
            <numerusform>%n active connection to ADON network</numerusform>
            <numerusform>%n active connections to ADON network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `adon_en.ts` remains unchanged.
